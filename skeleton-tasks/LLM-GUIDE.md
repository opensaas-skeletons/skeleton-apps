...
| `shared/types/task.ts` | Core TypeScript interfaces | ⚠️ MODIFY WITH CARE |
...
| `server/src/db/connection.ts` | PostgreSQL connection pool | ⚠️ MODIFY WITH CARE |
| `server/src/db/migrate.ts` | Migration runner | ⚠️ MODIFY WITH CARE |
| `server/src/db/seed.ts` | Sample data seeding | ✅ SAFE TO MODIFY |
| `server/src/db/migrations/001_initial.ts` | Initial schema | ⚠️ MODIFY WITH CARE |
...
| `server/tsconfig.json` | Server TypeScript config | ⚠️ MODIFY WITH CARE |
| `client/tsconfig.json` | Client TypeScript config | ⚠️ MODIFY WITH CARE |
